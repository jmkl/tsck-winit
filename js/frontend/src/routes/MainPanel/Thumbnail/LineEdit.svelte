<script lang="ts">
	...
	import { cicleRange, ScaleRange, setCaretPos, type TemplateLine } from './stringUtils';
				...
				{#each texts as text, index}
					<div
						...
						class="flex {text.include
							? ''
							: 'italic opacity-20'}  w-full flex-row items-center gap-1 border-b border-error/10 px-2"
					>
						<a
							tabindex="-1"
							onblur={() => {}}
							...
							ondblclick={(e) => {
								...
								editMode = !editMode;
								setTimeout(() => {
									const cursor = document.caretPositionFromPoint(x, y)?.offset ?? 0;
									if (temptEdittext) setCaretPos(temptEdittext, cursor);
								}, 100);
							}}
							class="ml-text line-clamp-1 flex-1 hover:text-error"
						>
							{text.text}
						</a>
						<button
							...
							class="{text.italic
								? 'italic'
								: 'normal'} btn h-3.75 w-3.75 rounded-xs p-0 text-[10px] btn-soft btn-error"
						>
							I
						</button>
						<button
							...
							onclick={(e) => {
								...
								text.scale = cicleRange(text.scale + denominator, 0, ScaleRange.length - 1);
							}}
							class=" btn h-3.75 w-6.25 rounded-xs p-0 text-[10px] btn-soft btn-error"
						>
							{text.scale}
						</button>
					</div>
