...
{#snippet renderMessage(text: string)}
	...
	{#each msg.urls as url}
		<a
			...
			class="btn absolute top-px left-px p-0.5 text-[8px] btn-soft btn-xs btn-error"
			href={url}
		>
			link
		</a>
...
<div
	bind:this={menuEl}
	class="absolute z-1000 flex w-auto flex-col gap-2 rounded-md bg-base-300 p-1 shadow-[0_2px_2px_black]"
>
	<button
		tabindex="-1"
		class="btn justify-end text-[10px] btn-ghost btn-error *:size-4"
		onclick={async (e) => {
...
{#if clipboarHelperDialog}
	<div
		...
		class="w-full {ctx.globalCompactMode
			? 'h-45'
			: 'h-62.5'} sticky z-999 flex items-center justify-center p-4"
	>
		<div
			class="relative flex h-full w-full flex-col overflow-y-auto rounded-md bg-base-200 p-4 font-jetbrains text-[8px] shadow-[0_0px_0px_2px_var(--color-error),0_5px_10px_0px_rgba(0,0,0,.1)] select-auto"
		>
			<div class="absolute top-2 right-4">
				<IconButton
					class="btn-square h-6! w-6!  p-1! btn-soft btn-error"
					onclick={() => {
						ctx.todoHelper?.addTodoFromClipboard(clipboardTexts.join('\n')).then(() => {
							ctx.todoUpdate();
			...
			{#each clipboardTexts as clipboard, index}
				<div class="flex w-full flex-col">
					<div class="rounded-sm px-1 py-1 odd:bg-base-300">
						{clipboard}
					</div>
					<button
						title="something"
						...
						class="separator pointer-events-auto h-1.25 w-full cursor-n-resize text-transparent hover:bg-error"
					>
						mush
					</button>
				</div>
...
<div
	...
	tabindex="-2"
	class="todolist flex h-full min-h-0 w-full flex-col overflow-x-hidden overflow-y-auto border-error p-0 [overflow-anchor:none] focus:outline-0"
>
	{#if ctx.todoList}
		{#each filteredTodo as todo, index}
			<div
				...
				class="flex h-auto {activeTodo == index
					? ''
					: ''} group relative w-full flex-row items-center border-error transition-all odd:bg-base-200"
			>
				<div
					bind:this={cursor}
				...
				{#if todo.status == TodoStatus.Done}
					<SmallCheckMark size={14} class="absolute left-1 text-error" />
				{/if}
				<a
					tabindex="-1"
					href="/"
					...
					1
						...
						: ' mask-[linear-gradient(to_bottom,black_60%,transparent)] px-4 py-2'} line-clamp-1 flex-1 wrap-break-word group-hover:text-neutral-content"
				>
        ...
        >
          {@render renderMessage(todo.message)}
				...
				<div
					class="absolute right-1 flex h-full flex-row items-center justify-center gap-1 bg-base-300 opacity-0 backdrop-blur-sm transition-opacity duration-500 ease-in-out group-hover:opacity-100"
				>
					<IconButton
						...
						class="h-4! w-4! p-0.5! text-error btn-error"
					/>
