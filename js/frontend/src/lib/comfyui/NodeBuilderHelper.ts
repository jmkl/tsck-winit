...
export function BuildFaceRestoreNode(
  ...
  return new Promise((resolve, reject) => {
    const loadImageNode = new LoadImage("2").setImage(image);
    // const loadImageNode = new LoadResizeImageMask512Node('2').setImage(image).setValue(512);

    const upscale_loader = new UpscaleModelLoaderNode("8").setModelName(
      upscaleModel,
    );
    ...
    const RMBG = new RMBGNode("1")
      .setModel(REMBGModel)
      .setImage([upscaleImagewModel.id, 0]);

    const adPixel = new AddPixelNode("3")
      .setImage(RMBG.id, 0)
      .setMask(RMBG.id, 1);
